...
function slider(container){
    ...
    */
    ...
    this.touchHandle = function(e,type){
        ...
        var y = touch[focusedTouch].screenY
        if(this.formalX == undefined){this.formalX = x}
        if(this.formalY == undefined){
            this.formalY = y
        }
        var dx = x - this.formalX
        var dy = y - this.formalY
        this.formalX = x
        this.formalY = y
        if(type == "start"){
            this.touching = true
        ...
        /*
        check for scrolls not meant to scroll the slider but to probably scroll the page, this will be done using the slope of the touch scroll calculated as dy/dx
        */
        if(dx != 0){
            var slope = Math.abs(dy/dx);
            var slopeAngle = Math.atan(slope) * (180/Math.PI)
            if(slopeAngle > 20){return}
        }
        stopDefault(e)
